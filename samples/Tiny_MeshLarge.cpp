...
void createPipelines() {
  ...
  const lvk::VertexInput vdescs = {
      .attributes = {{.format = VK_FORMAT_R32G32B32_SFLOAT, .offset = offsetof(VertexData, position)}},
      .inputBindings = {{.stride = sizeof(VertexData)}},
  };
...
void render(double delta) {
  ...
  if (isShadowMapDirty_) {
    ...
    buffer.cmdTransitionLayout({fbShadowMap_.depthStencil.texture}, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, {});
...
void showTimeGPU() {
  ...
  auto Sparkline = [](const char* id, const float* values, int count, float min_v, float max_v, const ImVec4& col, const ImVec2& size) {
    ...
    if (ImPlot::BeginPlot(id, size, ImPlotFlags_CanvasOnly)) {
      ...
      ImPlot::PlotShaded(id, values, count, 0, 1, 0, ImPlotSpec(ImPlotProp_LineColor, col, ImPlotProp_FillAlpha, 0.40f));
