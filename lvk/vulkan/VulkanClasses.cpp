...
void lvk::CommandBuffer::cmdBindIndexBuffer(BufferHandle indexBuffer, IndexFormat indexFormat, uint64_t bufferOffset, uint64_t bufferSize) {
  ...
  const VkBuffer vkBuffer = buf ? buf->vkBuffer_ : VK_NULL_HANDLE;
  ...
  if (ctx_->workaround_noMaintenance5_) {
    ...
    vkCmdBindIndexBuffer(wrapper_->cmdBuf_, vkBuffer, bufferOffset, type);
  } else {
    vkCmdBindIndexBuffer2KHR(wrapper_->cmdBuf_, vkBuffer, bufferOffset, bufferSize, type); // TODO: remove KHR to update to Vulkan 1.4
  }
}
...
#undef UPDATE_PUSH_CONSTANT_SIZE
  ...
  lvk::VulkanPipelineBuilder()
      ...
      .shaderStage(
          lvk::getPipelineShaderStageCreateInfo(VK_SHADER_STAGE_FRAGMENT_BIT, fragModule->ci, desc.entryPointFrag, &si, fragModule->sm))
